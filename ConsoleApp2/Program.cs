...
//custer first then catagorize
using DotNetEnv;
using System.IO;
using System.Text.Json;
using Octokit;
LoadEnv();
const int PROCESSED_COUNT = 10; // Adjust this value as needed to control how often the intermediate results are saved
...
static async Task<Dictionary<string, List<CommitInfo>>> FetchRepoCommitsAsync(GitHubClient github, string orgName, string filePath, int? maxRepos = null)
{
...
static async Task WriteRepoCommitsAsync(Dictionary<string, List<CommitInfo>> repoCommits, string filePath)
{
...
static bool ConfigureAuthentication(GitHubClient github)
{
...
static (string Title, string Description) SplitCommitMessage(string message)
{
